...
| フレームワーク | Hono（最新安定版） | ルーティング / cookie / ミドルウェア |
...
- CSRF 対策: 署名 cookie は `SameSite=Lax` + 変更系は同一オリジンの POST のみ許可 + 隠しトークン（`app_meta` 由来の per-session トークン）。
...
- **PIN 保存**: `pbkdf2$210000$<salt>$<hash>`（PBKDF2-SHA256, 210k 反復, 16byte ソルト）。
...
- **セッション cookie**:
  ...
  - 属性: `HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=7776000`。
  - 検証: 署名一致 + `exp` 未超過 + `user.active=1`。
- **CSRF トークン**: ログイン時に per-session ランダムトークンを cookie とは別に発行し、
  変更系フォームに隠しフィールドで埋め込み・照合。
