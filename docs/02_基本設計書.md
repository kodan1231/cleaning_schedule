...
| メソッド | パス | 説明 |
|---|---|---|
| POST | `/cleanings/:id/photos` | multipart。`full`（必須）,`thumb`（任意）,`item_id`,`kind`,`room_name`,`caption`。`item_id` 指定時は `kind='item'`。`kind=start` 指定時は現状撮影（一括管理・部屋の指定は不要。`room_name` は将来 部屋別に戻す場合のための任意項目で、送る場合はこの清掃のチェックリストに実在する間取り名のみ許可） |
