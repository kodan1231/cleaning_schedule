...
| メソッド | パス | 説明 |
|---|---|---|
| POST | `/cleanings/:id/photos` | multipart。`full`（必須）,`thumb`（任意）,`item_id`,`room_name`,`caption`。`item_id` 指定時は `kind='item'`、`room_name` 指定時（現状撮影）は `kind='start'`（`room_name` はこの清掃のチェックリストに実在する間取り名のみ許可） |
...
| 2026-09-23 | 2.0 | ①**作業開始時の現状撮影**（マイグレーション 0011、schema_version=11）。`photo` に `kind`（`item`/`start`）・`room_name` を追加。清掃詳細の「清掃を開始」ボタン下に部屋ごとのカメラボタンを追加（`POST /cleanings/:id/photos` に `room_name` を追加、`item_id` 未指定時に使用）。撮った写真は下部の「写真」欄にも間取り単位で表示され、サムネイルに「現状」バッジを表示。②**古い写真の自動圧縮**（`src/lib/photoRetention.js`）。アップロードから30日超の写真は `full` の BLOB のみ削除しサムネイルは保持。毎日の Cron 同期の最後に実行し、結果を `sync_log`（`property_id=NULL`）に記録。閲覧時は `full` が無ければ `thumb` にフォールバック |
