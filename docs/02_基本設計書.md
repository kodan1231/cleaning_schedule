...
[Cloudflare Workers]  ── Hono（ルーティング / 認証 / SSR）
   ...
   │            └─▶ [R2]  写真バイナリ（フル / サムネイル）
   │
[Cron Trigger（1日1回 JST 6:00）] ─▶ Worker の scheduled ハンドラ ─▶ iCal 取得 → D1 反映
   │
...
| Cron Triggers | 無料プランに含まれる | 1日1回 | $0 |
...
| ID | 画面 | パス | 権限 | 概要 |
|---|---|---|---|---|
| S-01 | ログイン | `/login` | 公開 | 表示名テキスト入力 + PIN |
...
| S-13 | 備品 | `/supplies` | member（マスタ編集は admin） | 物件ごとの消耗品ストック一覧・在庫数の更新 |
...
-- 作業イベントログ（実作業時間の計測・履歴。追記のみ・不変。マイグレーション 0002）
CREATE TABLE cleaning_event (
  id          INTEGER PRIMARY KEY,
  cleaning_id INTEGER NOT NULL REFERENCES cleaning(id) ON DELETE CASCADE,
  kind        TEXT    NOT NULL,   -- start|complete|reopen|note|check|uncheck|photo_add|photo_delete
  user_id     INTEGER REFERENCES user(id),
  at          TEXT    NOT NULL,   -- ISO8601 UTC
  detail      TEXT               -- 任意（項目ラベル等）
);
CREATE INDEX idx_cleanevt ON cleaning_event(cleaning_id, at);
-- 実作業時間 = start/reopen〜complete の各区間の合計（作業中なら現在時刻まで）。src/lib/events.js
...
CREATE TABLE supply (
  ...
  sort_order  INTEGER NOT NULL DEFAULT 0,
  stock       INTEGER NOT NULL DEFAULT 0, -- 現在の在庫数（最新の supply_log のスナップショット）
  note        TEXT,                       -- 最新更新時の備考
  updated_by  INTEGER REFERENCES user(id),
  updated_at  TEXT,
  created_at  TEXT    NOT NULL
);
CREATE INDEX idx_supply_property ON supply(property_id, sort_order);
...
CREATE INDEX idx_supply_log_supply ON supply_log(supply_id, changed_at DESC);
...
| POST | `/cleanings/:id/note` | 全体メモ更新（`property.memo`。次回以降の清掃にも表示。`cleaning.note` にも同内容を残し履歴・CSV 用に参照） |
| POST | `/cleanings/:id/rooms/:rid/memo` | 部屋の引き継ぎメモ更新（`room.memo`。次回以降の清掃にも表示。`rid` は当該物件の間取りのみ） |
...
| メソッド | パス | 説明 |
|---|---|---|
| POST | `/cleanings/:id/photos` | multipart。`full`（必須）,`thumb`（任意）,`item_id`,`kind`,`room_name`,`caption`。`item_id` 指定時は `kind='item'`。`kind=start` 指定時は「掃除前」の写真（一括管理・部屋の指定は不要。`room_name` は将来 部屋別に戻す場合のための任意項目で、送る場合はこの清掃のチェックリストに実在する間取り名のみ許可） |
...
| メソッド | パス | 説明 |
|---|---|---|
| GET | `/supplies` | 一覧（`?property=` で絞り込み。member 可） |
| POST | `/supplies` | 備品を追加（`property_id`,`name`,`unit`,`stock`。**admin のみ**） |
| POST | `/supplies/:id/stock` | 在庫数の更新（`stock`,`note`。**全メンバー可**。`supply_log` に履歴を残す） |
| POST | `/supplies/:id` | 名前・単位の編集（**admin のみ**） |
| POST | `/supplies/:id/move` | 並べ替え（`dir=up\|down`。**admin のみ**） |
| POST | `/supplies/:id/delete` | 削除（履歴 `supply_log` も CASCADE で削除。**admin のみ**） |
...
5. 予約ごとに `reservation` を upsert（キー: `property_id + ical_uid`）。
   - `checkout_date = DTEND`（Airbnb はチェックアウト日そのものを入れる。§5.3 で検証済み）。
   - `guest_hint`: `DESCRIPTION` から電話末尾4桁など抽出できれば格納。
   - **既存レコードは内容（`checkin/checkout/guest_hint/status/raw_text`）に変化がある時だけ全カラム UPDATE**。
     変化が無ければ `synced_at`（最終同期確認時刻）のみ更新し、D1 書き込みを抑える。
...
7. 予約 → 清掃生成:
   ...
   - 未着手・作業中の清掃のチェックリスト再構成（`resnapshotPending` / `mergeSnapshot`）も、
     項目の `label`/`needs_photo`/`note`/位置が実際に変わった時だけ UPDATE/INSERT/DELETE を発行する。
8. `sync_log` に結果を記録。予約・清掃・チェックリストのいずれにも変更が無かった回は `result='no_change'`。

### 5.2 スケジュールと冪等性

- Cron: `0 21 * * *`（UTC。JST 6:00・1日1回）。手動同期も同じ関数を呼ぶ。
- すべて upsert / 存在チェックで冪等。二重生成は `cleaning` の UNIQUE 制約で防止。
- 生成対象の日付範囲: `今日 - 3日` 〜 `今日 + 180日`。
- **D1 書き込みの差分化（2026-09-14）**: 同期のたびに全予約・全清掃を無条件で UPDATE していたのを、
  値が変わった時だけ書き込むように変更。1回あたりの無駄な書き込みを減らす方針とした。
- **同期間隔を1日1回に変更（2026-09-23）**: 3時間ごと（1日8回）から、清掃開始前の
  JST 6:00・1日1回に変更（§1.4）。同日中の予約変更・キャンセルは翌朝まで反映されない点に注意。

### 5.3 実データ確認結果（2026-09-08・実フィードで検証済み）

- `SUMMARY`: 予約 = `Reserved`（完全一致）/ ブロック = `Airbnb (Not available)`。ブロックは `DESCRIPTION` 行なし。
- `DESCRIPTION`（予約のみ・1行を折返し）:
  `Reservation URL: https://www.airbnb.com/hosting/reservations/details/<CODE>\nPhone Number (Last 4 Digits): <4桁>`
  （`\n` はリテラル）。`guest_hint` には電話下4桁のみ格納。
- `DTEND` = **チェックアウト日そのもの**（例: 1泊 `DTSTART:20260912` / `DTEND:20260913`）。`checkout_date = DTEND` を無変換で採用。
- 全イベントが `DTSTART;VALUE=DATE` / `DTEND;VALUE=DATE` の終日。TZID 指定なし。
- `STATUS` / `CANCELLED` 行は出力されない → 予約キャンセルは「同期時に iCal から消滅」でのみ検知（§3.2）。
- パーサ実装: `src/ical/parser.js`（`parseEvents` / `classifyEvent`）。
...
- **登録の締め切り**:
  - `app_meta.registration_open`（既定 `'1'`）が `'0'` のときだけ受付停止。
  - **人数上限は設けない**（2026-09-08 ユーザー方針。`max_users` は不使用。自動締め切りなし）。
  - admin は `/admin/users` から受付を再開/停止できる。
- **表示名の変更**: admin は `/admin/users` で各ユーザーの表示名を変更できる（`POST /admin/users/:id/name`、重複不可）。
- 未登録状態で認証必須ページにアクセス → `/register` へ誘導（token 未所持なら token 入力を促す）。

### 6.2 ログイン・セッション

- **ログイン**: 表示名を各自テキスト入力 + PIN（2026-09-08 ユーザー方針で選択式→入力式に変更）。
  名前が見つからない場合も「ユーザーまたは PIN が違います」で存在を隠す。
...
- **古い写真の圧縮（2026-09-23）**: アップロードから30日を超えた写真は `photo_blob` の
  `kind='full'` 行だけを削除し、`thumb` は残す（`src/lib/photoRetention.js`・毎日の Cron 同期の
  最後に実行）。閲覧時に `full` が無ければ `thumb` にフォールバックして表示する（`src/photos.js`）。
  対象は清掃の写真（`photo`）のみで、間取りの参考写真（`room_photo`）は対象外。
...
```jsonc
{
  ...
  // ],
  "triggers": { "crons": ["0 21 * * *"] },  // JST 6:00・1日1回
...
| NFR-5 コスト | §1.3。縮小保存、Cron 1日1回、無料枠内・カード登録なし |
...
| D2 | Airbnb iCal の実フィールド値 | ✅ 2026-09-08 実フィードで検証済み（§5.3）。残: 予約キャンセルの実挙動のみ未確認 |
...
| 2026-09-08 | 0.6 | P3 実装。§5.3 を実フィード検証結果に更新（SUMMARY/DTEND/DESCRIPTION 確定）、§12 D2 を解決済みに |
| 2026-09-08 | 0.7 | ユーザー方針: 人数上限撤廃・ログインを表示名入力式に。表示名変更（admin）。清掃削除（admin）。`cleaning_event` テーブル追加（実作業時間の計測、マイグレーション 0002） |
| 2026-09-08 | 0.8 | P6 実装。写真は D1 BLOB（`OffscreenCanvas` は使わず `<img>`+`<canvas>`）。**FR-17（必須写真ガード）は撤回**（ユーザー方針: 写真はオプション）。`cleaning_event` に photo_add/photo_delete |
| 2026-09-08 | 0.9 | P7 PWA（SW キャッシュ戦略・`/offline`・アイコン）。P8 仕上げ（`/history` + CSV/JSON エクスポート、`/admin` 写真容量表示、404/500 ページ）。**P0〜P8 完了** |
| 2026-09-08 | 1.0 | 間取りモデル導入（マイグレーション 0003）。物件 1-N 間取り、間取り 1 テンプレート。`room` テーブル追加、`checklist_template_item.area_label` 削除、`checklist_item` を `room_name`/`room_sort` 化。テンプレは名前付き共有プール（`is_base`/`property_id` 不使用）。清掃生成時に物件の全間取りを展開 |
| 2026-09-08 | 1.1 | 管理フロー改善: 物件登録をダイアログ化（`/admin/properties/new` フォールバック）、間取りをコンパクト1行レイアウトに。**FR-10 の「スナップショット不変」を緩和**: `pending` かつチェック未着手の清掃は、iCal 同期時に現在の間取り／テンプレで作り直す（「後から間取りを設定 → 同期で反映」を可能に）。物件詳細に「チェックリストを再生成」（未完了清掃対象・force） |
| 2026-09-08 | 1.2 | SW を v2 に（app.js/css を network-first に。古い JS 配信でトグルが失敗する不具合を修正）。間取りのドラッグ並べ替え。清掃詳細のチェックリストを圧縮＋折りたたみ: 間取りごとに `<details>`（全項目チェックで自動折りたたみ）、`room.group_label`（階など・マイグレーション 0004）で階セクションも折りたたみ。写真は行内カメラアイコンから追加（常時ドロップゾーン廃止） |
| 2026-09-08 | 1.3 | 宿泊日程（チェックイン〜アウト・泊数）をカレンダー／清掃詳細／履歴に表示（`reservation.checkin_date`）。**間取りグループ（階）機能を廃止**（間取りごとの開閉で不要に）。`room.group_label`／`checklist_item.room_group` は未使用で残置、読み書きコードは削除 |
| 2026-09-08 | 1.4 | **間取りごとの追加チェック項目**（マイグレーション 0005 `room_item`、schema_version=5）。共有テンプレに含めない部屋固有の項目を間取りに直接ぶら下げる。`snapshotChecklist` は各間取りで〈テンプレ項目 → room_item〉の順に展開。物件詳細の各間取り行に折りたたみ「＋ この部屋だけの項目」（追加/編集/削除/↑↓） |
| 2026-09-13 | 1.5 | 運用フィードバックによる UX 改善: ①**間取りの参考写真**（`room_photo`/`room_photo_blob`・マイグレーション 0006）を複数枚＋キャプションで登録、清掃チェックリスト画面にも表示。②**チェックリスト再生成をマージ方式に変更**（`resnapshotPending`。`force`/`includeInProgress` 廃止、`pending`＋`in_progress` を常に対象、チェック済み項目は室名＋`item_key` で対応付けて保持）。`room_item` に安定キーを追加（マイグレーション 0007、schema_version=7）。③テンプレ項目／room_item の↑↓並べ替えボタンを `<details>` の外に常時表示化。④間取りの並べ替えをドラッグから↑↓ボタンに一本化（`POST /admin/properties/:id/rooms/reorder` 削除） |
| 2026-09-14 | 1.6 | **iCal 同期の D1 書き込みを差分化**（同期間隔 3時間ごとは維持）。`reservation` の UPDATE と `resnapshotPending`/`mergeSnapshot` を、値が実際に変わった項目だけ書き込むように変更。`sync_log.result` に `no_change`（変更なし）を追加（マイグレーション 0008、schema_version=8）。管理画面の同期結果表示・メッセージを `ok`/`no_change`/`error` の3値に対応 |
| 2026-09-21 | 1.7 | **部屋ごとの引き継ぎメモ**（マイグレーション 0009、schema_version=9）。`room` に `memo`/`memo_updated_by`/`memo_updated_at` を追加。清掃詳細の各部屋ブロック内で編集し、清掃ごとのスナップショットではなく `room` に直接保存するため次回以降の清掃にも自動で引き継がれる（最後に保存した内容が有効）。メモのある部屋は見出しに「📝メモあり」を表示。作業記録に `room_memo`（部屋メモ更新）を追加。従来の清掃単位メモ（`cleaning.note`）は「この回だけの記録」として存続 |
| 2026-09-21 | 1.8 | **全体メモも引き継ぎ対応**（マイグレーション 0010、schema_version=10）。`property` に `memo`/`memo_updated_by`/`memo_updated_at` を追加し、清掃詳細上部の全体メモを物件単位で保存（`property.note` は管理画面用で別物）。保存時は `cleaning.note` にも同内容を残し履歴・CSV は従来どおり。既存の清掃メモは物件ごとに最新1件を初期値として移行。1.7 で「この回だけ」とした清掃メモの位置づけは廃止 |
| 2026-09-23 | 1.9 | **iCal 同期の間隔を3時間ごと→1日1回に変更**（Cron を `0 */3 * * *` → `0 21 * * *`＝JST 6:00 に変更。清掃開始〈午後〉より前に前日分の予約変更・キャンセルを反映）。同日中の変更は翌朝まで反映されない点に注意 |
| 2026-09-23 | 2.0 | ①**掃除前の写真**（マイグレーション 0011、schema_version=11）。`photo` に `kind`（`item`/`start`）・`room_name`（将来の部屋別運用に備えた予備項目。現状は未使用）を追加。清掃詳細の「清掃を開始」ボタン下に「掃除前」セクションを追加し、部屋を分けず一括で何枚でも撮れるようにした（`POST /cleanings/:id/photos` に `kind=start` を追加、`item_id` 未指定時に使用）。撮った写真は下部の「写真」欄（チェック項目の証跡写真）とは別に専用セクションでまとめて表示。②**古い写真の自動圧縮**（`src/lib/photoRetention.js`）。アップロードから30日超の写真は `full` の BLOB のみ削除しサムネイルは保持。毎日の Cron 同期の最後に実行し、結果を `sync_log`（`property_id=NULL`）に記録。閲覧時は `full` が無ければ `thumb` にフォールバック |
| 2026-09-23 | 2.1 | **備品（消耗品ストック管理）機能を新設**（マイグレーション 0012、schema_version=12）。物件ごとに `supply`（備品マスタ・現在庫のスナップショット）と `supply_log`（更新履歴・追記のみ）を追加。タブバーに「備品」を追加（`/supplies`、S-13）。在庫数の更新（数値入力＋備考欄）は全メンバー可、備品マスタ（名前・単位）の追加・編集・削除・並べ替えは admin のみ。在庫下限の警告表示は対象外（将来検討） |
